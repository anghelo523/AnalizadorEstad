...
public class RegressionAnalysisController {
    ...
    private void displayRegressionResults(Map<String, Object> results) {
        ...
        sb.append("Resultados de la Regresión Lineal Múltiple\n\n");
        sb.append("Dataset: ").append(currentDataset.getName()).append("\n");
        sb.append("Variable Dependiente (Y): ").append(dependentVariableComboBox.getSelectionModel().getSelectedItem()).append("\n");
        sb.append("Variables Independientes (X): ").append(String.join(", ", independentVariablesListView.getItems())).append("\n");
        sb.append("Número de Observaciones: ").append(results.get("Num Observations")).append("\n");
        sb.append("Número de Variables Independientes: ").append(results.get("Num Independent Variables")).append("\n\n");

        sb.append("Estadísticas del Modelo:\n");
        sb.append(String.format("R-cuadrado (R²): %.4f\n", (Double) results.get("R-Squared")));
        sb.append(String.format("R-cuadrado Ajustado: %.4f\n", (Double) results.get("Adjusted R-Squared")));
        sb.append(String.format("Error Estándar de la Regresión (Sigma): %.4f\n", (Double) results.get("Regression Standard Error (Sigma)")));
        sb.append(String.format("Suma de Cuadrados de la Regresión (SSR): %.4f\n", (Double) results.get("Regression Sum of Squares (SSR)")));
        sb.append(String.format("Suma de Cuadrados de los Residuales (SSE): %.4f\n", (Double) results.get("Residual Sum of Squares (SSE)")));
        sb.append(String.format("Suma Total de Cuadrados (SST): %.4f\n\n", (Double) results.get("Total Sum of Squares (SST)")));
